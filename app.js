...
class Triangle {
  constructor(size, origin, rotation) {
    this.a = new Coordinate(origin.x, origin.y - 500);
    this.b = new Coordinate(Math.cos((rotation * -1) * (Math.PI / 180)) * size + origin.x, Math.sin((rotation * -1) * (Math.PI / 180)) * size + origin.y - 500);
    rotation += 60;
    this.c = new Coordinate(Math.cos((rotation * -1) * (Math.PI / 180)) * size + origin.x, Math.sin((rotation * -1) * (Math.PI / 180)) * size + origin.y - 500);
  }
}
